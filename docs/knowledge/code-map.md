...
| Pet reacting to Claude/Codex turn-completed / needs-approval (task 0017, extended to Codex in task 0027) | `src-tauri/src/watcher/agent_status.rs` (hook-file tailer, provider-agnostic) → `lib.rs` `start_agent_status_notify_watcher()` (emits `agent_status_changed`) → `ui/overlay/src/state.ts` `agentStatusBadge` → `render.ts` `drawAgentStatusBadge()`; see [[agent-status-notifications\|Agent Status Notifications]] |
| Shared atomic JSON hook-settings read/write/marker plumbing (task 0027) | `src-tauri/src/hook_settings.rs` - used by both `claude_hooks.rs` and `codex_hooks.rs` |
| Installing/removing the Claude Code `Stop`/`PermissionRequest`/`PostToolUse`/`PermissionDenied` hooks (dashboard "Install hook" / "Remove hook" buttons) | `src-tauri/src/claude_hooks.rs` (`MANAGED_HOOKS`, `status()`/`install()`/`uninstall()`) → `lib.rs` `agent_status_hook_status`/`install_agent_status_hooks`/`uninstall_agent_status_hooks` commands → `ui/dashboard/src/routes/+page.svelte` (`agentStatusHookStatus`, `installAgentStatusHooks()`/`uninstallAgentStatusHooks()`); see [[agent-status-notifications\|Agent Status Notifications]] |
| Installing/removing the Codex CLI `Stop`/`PermissionRequest`/`PostToolUse` hooks (task 0027, dashboard "Install hook" / "Remove hook" buttons under Codex CLI hook) | `src-tauri/src/codex_hooks.rs` (`MANAGED_HOOKS`, `status()`/`install()`/`uninstall()`) → `lib.rs` `codex_hook_status`/`install_codex_hooks`/`uninstall_codex_hooks` commands → `ui/dashboard/src/routes/+page.svelte` (`codexHookStatus`, `installCodexHooks()`/`uninstallCodexHooks()`); see [[agent-status-notifications\|Agent Status Notifications]] |
